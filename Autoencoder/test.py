...
def custom_loss(output, target, alpha=1.0, beta=0.5):
    """
    Custom loss function that combines L1 loss for mortality rates and
    KL divergence + Wasserstein distance for the predicted lognormal parameters.
    
    Args:
        output (tensor): The model's output, where the last three entries are
                         the predicted lognormal parameters (shape, loc, scale).
        target (tensor): The ground truth target, where the last three entries
                         are the actual lognormal parameters (shape, loc, scale).
        alpha (float): Weight for the L1 loss.
        beta (float): Weight for the KL divergence and Wasserstein distance.
        
    Returns:
        loss (float): The combined loss.
    """
    # Split mortality rates and distribution parameters
    mortality_output, lognorm_output = output[:-3], output[-3:]
    mortality_target, lognorm_target = target[:-3], target[-3:]
    
    # L1 loss for the mortality rates
    l1_loss = nn.L1Loss()(mortality_output, mortality_target)

    # KL divergence and Wasserstein distance for lognormal parameters
    kl_div = kl_divergence_lognorm(*lognorm_target, *lognorm_output)
    w_dist = wasserstein_distance_lognorm(*lognorm_target, *lognorm_output)

    # Combine the losses
    total_loss = alpha * l1_loss + beta * (kl_div + w_dist)
...
def train_model(train_loader, val_loader, model, optimizer, scheduler, num_epochs=NUM_EPOCHS, patience=PATIENCE, alpha=1.0, beta=0.5):
    ...
    for epoch in range(num_epochs):  # Epoch loop
        ...
        for inputs, targets in train_loader:  # For each batch
            ...
            # Use custom loss
            loss = custom_loss(outputs, targets, alpha=alpha, beta=beta)
        ...
        with torch.no_grad():
            for val_inputs, val_targets in val_loader:
                ...
                # Use custom loss for validation
                val_loss += custom_loss(val_outputs, val_targets, alpha=alpha, beta=beta).item()
