...
def load_yearly_mortality(year):
    input_path = f'Revisions/hepvu_mort_rates_censored.csv'
...
def fill_missing_neighbors(mort_df, neighs_df, year, num_missing):
    ...
    for fips, row in mort_df.iterrows():
        if row[f'{year} OD'] == -9.0:
            ...
            available_neighbors = [neighbor for neighbor in neighbors if neighbor in mort_df.index and mort_df.loc[neighbor, f'{year} OD'] != -9]
...
def clean_rates(mort_df, neighs_df, year):
    ...
    mort_df[f'{year} OD'] = mort_df[f'{year} OD'].round(2).clip(lower=0)
...
def main():
    ...
    output_path = 'Revisions/hepvu_mort_rates_imputed.csv'
