...
# Set up logging
log_file = 'Log Files/xgboost_NMD.log'
...
def get_observed_mask_for_year(year, data_df):
    ...
    interim_df = interim_df[["FIPS", year_col]]

    # Merge with the main data_df so mask lines up with your feature matrix
    merged = data_df[["FIPS"]].merge(interim_df, on="FIPS", how="left")

    # Observed = not equal to -9.0
    observed_mask = merged[year_col] != MISSING_VALUE
...
def save_predictions(year, xgb_predictions, test_fips):
    ...
    saving_df.to_csv(f'XGBoost/XGBoost NMD Predictions/{year}_xgboost_NMD_predictions.csv', index=False)
...
def plot_feature_importance(feature_importance_df):
    ...
    ax.set_title('XGBoost NMD Feature Importance', fontsize=20, fontweight='bold')
    ax.legend(title='Year', fontsize=15, title_fontsize=15, loc='lower right')

    # Adjust layout and save
    plt.tight_layout()
    plt.savefig('Feature Importance/xgboost_NMD_feature_importance.png', bbox_inches='tight')
    plt.close()
...
def main():
    ...
    fips_codes_full = strip_fips(data_df)

    for year in range(2011, 2023):  # We start predicting for 2011, not 2010
        # Build full features / targets
        features, targets = features_targets(data_df, year)

        # Build observed-only mask using interim mortality data
        observed_mask = get_observed_mask_for_year(year, data_df)

        # Apply mask: keep only counties with observed (non-imputed) mortality
        features_year = features[observed_mask].reset_index(drop=True)
        targets_year = targets[observed_mask].reset_index(drop=True)
        fips_year = fips_codes_full[observed_mask].reset_index(drop=True)

        logging.info(
            f"{year}: Using {observed_mask.sum()}/3144 counties with observed mortality."
        )

        # Run XGBoost on the observed-only subset
        feature_importances, xgb_predictions, test_fips = run_xgboost(
            features_year, targets_year, fips_year
        )

        # Save predictions for this sensitivity analysis (note: only observed counties)
    ...
    # Final overall importance plot (sensitivity)
